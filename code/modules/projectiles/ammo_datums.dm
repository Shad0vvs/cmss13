...
/datum/ammo/rifle/dart/on_embed(var/mob/embedded_mob, var/obj/limb/target_organ)
...
/datum/ammo/rifle/dart/on_hit_obj(obj/O, obj/item/projectile/P)
	drop_dart(P.loc, P)
...
/datum/ammo/rifle/dart/explosive/drop_dart(var/loc, obj/item/projectile/P, var/mob/shooter)
...
/datum/ammo/rifle/dart/explosive/proc/delayed_explosion(obj/item/projectile/P, mob/M, mob/shooter)
	if(istype(M, /mob))
		...
		cell_explosion(get_turf(M), 50, 25, EXPLOSION_FALLOFF_SHAPE_LINEAR, P.dir, cause_data)
...
/datum/ammo/rifle/dart/track/on_hit_mob(mob/living/M, obj/item/projectile/P)
	if(!M || M == P.firer) return
	shake_camera(M, 2, 1)
	GLOB.sharp_tracked_mob_list |= M
	addtimer(CALLBACK(src, .proc/remove_tracker, M), tracker_timer)

/datum/ammo/rifle/dart/track/proc/remove_tracker(mob/living/M)
	GLOB.sharp_tracked_mob_list -= M
...
/datum/ammo/rifle/dart/flechette/on_hit_mob(mob/living/M, obj/item/projectile/P)
	if(!M || M == P.firer) return
	var/mob/shooter = P.firer
	shake_camera(M, 2, 1)
	if(shooter && ismob(shooter))
		if(!M.get_target_lock(shooter.faction_group))
			create_shrapnel(M.loc, min(direct_hit_shrapnel, shrapnel_count), P.dir, dispersion_angle, shrapnel_type, P.weapon_cause_data, FALSE, 100)
			shrapnel_count -= direct_hit_shrapnel
			if(shrapnel_count)
				create_shrapnel(M.loc, shrapnel_count, P.dir, dispersion_angle ,shrapnel_type, P.weapon_cause_data, FALSE, 0)
...
/datum/ammo/rifle/dart/flechette/proc/apply_explosion_overlay(var/turf/loc)
